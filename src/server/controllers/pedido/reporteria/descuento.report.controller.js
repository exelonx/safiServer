...
const getReporteDescuento = async (req = request, res = response) => {
    ...
    try {
        ...
        const descuento = await viewDescuento.findAll({
            where: {
        ...
        const descuentoMapped = descuento.map((descuentos) => {
            return {
                NOMBRE: descuentos.NOMBRE,
                CANTIDAD: descuentos.CANTIDAD,
                ID_TIPO_DESCUENTO: descuentos.ID_TIPO_DESCUENTO,
                DETALLE: descuentos.DETALLE,
                ES_PORCENTAJE: descuentos.ES_PORCENTAJE,
            }
        })

        const content = await compilarTemplate('descuento', { descuentos: descuentoMapped })
        ...
        const pdf = await pagina.pdf({
            ...
            <div style="display: flex; width: 60%; margin: 0 auto; align-items: center; justify-content: center; flex-direction: column"><div style="font-weight: bold; font-size: 20px;">${nombreEmpresa.VALOR}</div> <div style="color: #d12609; font-size: 20px;">Reporte de Descuentos</div></div>   
